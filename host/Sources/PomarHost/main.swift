...
let usage = """
    usage: pomar-host version
           ...
           pomar-host helper --attempt ID --state-dir DIR --store S --kernel K \\
                             ...
                             [--goproxy-socket SOCK --shim BIN] [--outputs NAME,NAME... --outputs-dir DIR --outputs-max BYTES] [--readonly-source yes] [--job-user yes] [--pins FILE] \\
...
case "helper":
    ...
    guard let readonlySource = Helper.yesFlag(f["readonly-source"]), let jobUser = Helper.yesFlag(f["job-user"]),
        !readonlySource || f["source"] != nil, f["pins"] == nil || f["source"] != nil
    else {
        fail("helper: --readonly-source and --job-user take yes; --readonly-source and --pins need --source", code: 2)
    }
    ...
    let code = await Helper.run(
        .init(
            ...
            readonlySource: readonlySource, jobUser: jobUser, pins: f["pins"],
